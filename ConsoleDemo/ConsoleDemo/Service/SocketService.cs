...
namespace ConsoleDemo.Service
{
    ...
    public class SocketService
    {
        ...
        public bool Start(IPAddress ip, int port, Action<SocketClient, bool> clientCallback, Action<SocketClient> responseCallback)
        {
            return Start(ip, port, () => { }, clientCallback, responseCallback);
        }
        ...
        public bool Start(IPAddress ip, int port, Action serviceCloseCallback, Action<SocketClient, bool> clientCallback, Action<SocketClient> responseCallback)
        {
            ...
            {
                server.Close();
                log.Error("socket服务器端口号冲突 或 未知错误");
                return false;
            }
